...
with open(r"dataset\\sensiAPI.txt",
          "r") as f:
    a = f.read().split(',')
...
def write_code_to_csv(data_dir: str, csv_path: str, symbloize: bool):
    header = ['file_path', 'code', 'label']
    csvfile = csv.DictWriter(open(csv_path, 'w', encoding='utf-8'), fieldnames=header)
    csvfile.writeheader()
    for file in os.listdir(data_dir):
        csv_data = {}
        csv_data['file_path'] = file
        data_file_path = os.path.join(data_dir, file)
        with open(data_file_path, 'r') as f:
            data = json.load(f)
            csv_data['label'] = data['label']
            if symbloize:
                cleaned = clean_gadget(data['node_code'])
            else:
                cleaned = data['node_code']
            tokens = []
            for code in cleaned:
                tokens.extend(tokenize_code_line(code, False))
            csv_data['code'] = ' '.join(tokens)
        csvfile.writerow(csv_data)
